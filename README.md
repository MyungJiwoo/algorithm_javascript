...
- P181912: 배열 만들기 5
  ...
  _문자열.substr(시작인덱스, 길이)_ 헷갈리지 말자!

- P181911: 부분 문자열 이어 붙여 문자열 만들기

  _문자열.slice(시작인덱스, 끝인덱스)_ 를 기억하자..
...
- P120826: 특정 문자 제거하기
  ...
  `replaceAll()`로 특정 문자를 모두 ""으로 치환했다.
...
- P120849: 모음 제거
  ...
  정규식에서 |으로 or을 사용했고, /g 옵션으로 모든 패턴을 골랐다. 그리고 replaceAll()이 아님에 주의해야 한다.

### 2025.05.05

- P120854: 배열 원소의 길이

  배열 메소드 `map()`에서 return으로 각 원소의 길이를 반환했다.

### 2025.05.07

- P120824: 짝수 홀수 개수

  정수는 짝수 아니면 홀수라는 점을 이용해서 계산으로 최소한으로 했다.
...
- P161990: 바탕화면 정리
  ...
  시도 1 - filter()를 사용해서 페인트가 시작되는 부분부터 한 번에 페인트 할 수 있는 부분까지를 거르는 방식으로 진행
  => while + filter로 시간이 너무 오래 걸림
  시도 2 - filter 부분을 인덱스를 사용한 방식으로 변경
...
- P120822: 뒤집힌 문자열
  ...
  주어진 문자열을 배열로 변환후 reverse() 내장 메소드로 뒤집은 뒤 join()으로 다시 문자열로 변환했다.
...
- P155652: 둘만의 암호
  ...
  [utf-16] a: 97 ~ z: 122
  (1) charCodeAt() : char -> utf-16
  (2) String.fromCharCode() : utf-16 -> char

### 2025.06.25

- P147355: 크기가 작은 부분 문자열

  `for...in`문에서 index를 역할하는 변수는 항상 string으로 처리된다.
  아무튼 관건이었던 건, 단순히 숫자 비교뿐만 아니라 자릿수가 p의 자릿수와도 같은지였다.
...
- P138477: 명예의 전당 (1)
  ...
  while문으로 직접 내림차순으로 배열 요소를 추가하도록 만들었다.
  최소 힙으로 명예의 전당 인원 수만 관리하면 효율적으로 풀 수 있다.
...
- P161989: 덧칠하기
  ...
  - 첫 시도 방법
    1. section 배열을 순회하면서, 첫 페인트가 시작될 인덱스를 painted에 기억한다.
    2. 그리고 filter를 통해 painted 한 번에 칠할 수 있는 인덱스들을 삭제한다.
    3. 한 번 덧칠할 때마다 count를 +1하여 결과를 반환한다.
  - 문제점 : while() 안에서 filter()로 모든 배열을 순회하려고 하니 시간 복잡도가 커져서, 시간 초과 문제가 발생했다.
  - 해결 방법 : filter()로 배열을 추출하는 부분을 인덱스 기반으로 수정했다.

### 2025.08.21

- P120836: 순서쌍의 개수

  문제와는 별개지만 자주 나오는 수학 개념을 코드의 흐름으로 연습하고 필요한 부분은 암기하는게 도움될 것 같다.

### 2025.08.23

- P120905: n의 배수 고르기

  문제의 조건을 보자마자 filter 함수가 생각났다.
  filter 함수의 결과는 얕은 복사라, 원본 배열에 영향을 주지 않는다.

### 2025.08.25

- P120819: 아이스 아메리카노

  문제는 쉬웠고, 다른 사람 풀이랑 비교했을 때 나는 가격을 상수로 처리해서 매직넘버를 줄였다.
...
- P120825: 문자 반복 출력하기
  ...
  1. for~in: 순회는 가능하지만 순서 보장이 되지 않는 문제점
  2. for~of: for~in의 문제점 해결 (순서 보장) + 각 문자를 반환 => String 순회에 적합
  3. [...str].map(): String을 Array로 변환하여 순회

### 2025.08.29

- P181877: 대소문자로 바꾸기

  str.toUpperCase() / str.toLowerCase()로 대소문자 변환

### 2025.09.05

- P120583: 중복된 숫자 개수

  배열에서 원하는 값을 모두 가져올려면 find(), includes(), ~Index() 모두 안된다. (boolean으로 반환하거나 원하는 원소의 첫 인덱스 정도만 반환해서)
  그래서 filter로 계산했다.
  그리고 기본적으로 .length() 같은 기본 메소드는 없으니 헷갈리지 말기!

### 2025.09.07

- P120809: 배열 두 배 만들기

  각 배열 원소에 2배씩 한 배열을 리턴하는데 나는 map으로 풀었고, 다른 사람의 풀이를 보니 reduce로 푸는 방법도 있었다.
  다만 [...a, b * 2]와 같은 방식은 매번 a를 전부 복사하기 때문에 메모리 낭비가 심할 수 있다고 한다.
...
- P120811: 중앙값 구하기
  ...
  배열의 길이가 홀수일 때, 중앙 인덱스 값을 구하려면, 배열의 인덱스는 0부터 시작하니까 length/2의 Math.floor로 버림을 해야 한다.

### 2025.09.26

- P120816: 피자 나눠먹기 (3)

  문제 자체는 어렵지 않았다. 그저 오랜만에 풀었더니 꾸준히 해야겠다고 생각했다.

### 2025.09.28

- P120841: 점의 위치 구하기

  좌표가 속한 사분면을 구하는 문제에서, 뻔한 if말고 사칙 연산으로 풀어보고 싶었다. 결국 나는 if문으로 풀었지만 다른 사람의 풀이를 보니 사칙연산과 if문을 섞어서 풀었다. 신기하긴 한데 가독성이 좋게 느껴지진 않았다.

### 2025.10.03

- P120813: 짝수는 싫어요

  원래는 while, for을 사용하지 않고 풀고 싶었지만 애초에 정수가 주어지다보니 오히려 배열 메소드로 푸는게 비효율적이라는 생각이 들었다. 그래서 for을 선택한 대신 항상 홀수만 출력하기 때문에 i+=2로 반복 횟수를 줄였다.

### 2025.10.05

- P120818: 옷가게 할인 받기

  나는 if, else if로 풀었고, 다른 사람의 풀이를 비교해도 다 비슷한 것 같다.

### 2025.10.07

- P120823: 직각 삼각형 출력하기

  js 입출력을 다뤄야해서 readline의 코드가 미리 주어졌고, 내가 특별히 입출력을 제한하지는 않았다.
  별찍기 문제를 오랜만에 풀었는데 repeat() 메소드를 사용해서 중첩 반복문 없이 풀 수 있었다.

### 2025.10.09

- P120839: 가위 바위 보

  나는 switch문을 사용해서 풀었는데 객체 매핑 방식도 좋은 것 같다.

### 2025.10.11

- P120837: 개미 군단

  문제를 보자마자 그리디 알고리즘이 생각났고, 개미 군단의 병력은 고정 값이라 ants 상수를 만들어서 나눗셈으로 계산했다.

### 2025.10.15

- P181832: 정수를 나선형으로 배치하기

  오랫동안 고민하다가 진도가 나가지 않아 인터넷을 참고했다.
  이중 for인데, depth=2에서 여러 for문을 순차적으로 나열했다는점이 새로웠다.
  다른 풀이를 보니 direction이나 [0, -1] 이런식으로 방향을 조정하는 것 같다.
  아쉬움도 많이 남는 문제다.
...
정규식으로 숫자 필터링 하는 것도 방법이다.

### 2025.11.20

- P12931: 자릿수 더하기

문자열로 바꿔서 푸는 방법과, 숫자 자체로 자릿수마다 출력하는 방법을 시도했다.
큰 차이는 없지만 숫자 자체로 자릿수마다 출력하는 방식이 알고리즘 학습에는 더 도움될 것 같다.

### 2025.11.26

- P12932: 자연수 뒤집어 배열로 만들기

`parseInt()`, `toString()`의 형변환 방식도 다양하게 알아두자.

### 2025.11.28

- P181853: 뒤에서 5등까지

`slice()`는 새로운 배열을 반환하고, `splice()`는 원본 배열을 수정한다.

### 2025.11.30

- P181854: 배열의 길이에 따라 다른 연산하기

나는 인덱스를 직접 계산하고 비교해서 풀었지만, map을 사용해서 선언적으로 풀 수 있었다. 삼항연산자와 함께...

### 2025.12.07

- P181863: rny_string

`replaceAll`을 사용해서 금방 풀었다.

### 2025.12.09

- P181869: 공백으로 구분하기 1

`split()`의 인자로 문자열과 정규식(!!)을 줄 수 있다.
...
- P181833: 특별한 이차원 배열 1
  ...
  `Array.from().fill()`에서 **fill의 매개변수로 객체를 넣으면 참조를 공유한다.**  
  따라서 배열 초기화를 시키고 싶다면 콜백 함수를 활용해야 한다.
